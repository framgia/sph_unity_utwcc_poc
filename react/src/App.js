...
function App() {
  const [shapeSize] = useState(0)
  const maxShapeSize = 100
  const { adjustScale } = scaler()
        ...
        <nav className="space-y-8 text-sm">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-col space-y-1">
              ...
              <input type="range" name="shape" step="1" min="0" max={maxShapeSize} defaultValue={shapeSize} className="w-full h-2 bg-blue-100 appearance-none" onChange={e => adjustScale(e, unityContext)}/>
            </div>
          </div>
